...
import com.gildedgames.the_aether.blocks.decorative.BlockSkyrootWall;
...
public class AetherGenMassiveTree extends WorldGenAbstractTree
{
	...
	public AetherGenMassiveTree(IBlockState leaves, IBlockState log, int heightWeight, boolean branchFlag, boolean notify)
	{
		this(leaves, heightWeight, branchFlag, notify);

		this.log = log;
	}

	public AetherGenMassiveTree(IBlockState leaves, int heightWeight, boolean branchFlag, boolean notify)
	{
		super(notify);
	...
	public boolean generate(World world, Random rand, BlockPos position)
	{
		...
		if (y + (height + 2) <= world.getActualHeight() && cangen)
		{
			...
			IBlockState wall = BlocksAether.skyroot_wall.getDefaultState().withProperty(BlockSkyrootWall.PROPERTY_GENERATED, true),
			...
			for (int y2 = y; y2 <= y + height; y2++)
			{
				...
				world.setBlockState(pos, this.log != null ? log : BlocksAether.aether_log.getDefaultState());
			}
			...
			if (this.branches)
			{
				IBlockState log = this.log != null ? this.log : BlocksAether.aether_log.getDefaultState();
