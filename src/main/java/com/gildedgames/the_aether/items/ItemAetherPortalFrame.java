...
public class ItemAetherPortalFrame extends Item {
    ...
    private boolean createPortalFrame(World world, int x, int y, int z, int facing) {
        final int posX = x;
        final int posY = world.getBlock(x, y, z)
            .isReplaceable(world, x, y, z) ? y + 1 : y + 2;
        final int posZ = z;
        ...
        for (int zi = 0; zi < 4; ++zi) {
            for (int yi = -1; yi < 4; ++yi) {
                ...
                if (world.getBlock(blockX, blockY, blockZ) != Blocks.air && !world.getBlock(blockX, blockY, blockZ)
