...
public class ComponentGoldenAercloud extends AetherStructure
{
	...
	public ComponentGoldenAercloud(Random random, int chunkX, int chunkY, int chunkZ)
	{
                ...
                for (int y1 = y; y1 < y + yMax; ++y1)
                {
                    for (int z1 = z; z1 < z + zMax; ++z1)
                    {
                        if (this.getBlockStateWithOffset(x1, y1, z1).getBlock() == Blocks.AIR)
                        {
                        	...
                        	{
                        		this.setBlockWithOffset(x1, y1, z1, BlocksAether.aercloud.getDefaultState().withProperty(BlockAercloud.cloud_type, EnumCloudType.Golden));
                        	}
                        }
                    }
                }
