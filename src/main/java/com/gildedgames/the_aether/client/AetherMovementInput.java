...
public class AetherMovementInput extends MovementInputFromOptions {
    ...
    public void updatePlayerMoveState() {
        ...
        if (this.mc.thePlayer.ridingEntity instanceof EntitySaddleMount) {
            if (this.mc.thePlayer.ridingEntity instanceof EntitySaddleMount) {
                ...
                if (this.previousSneak != this.currentSneak) {
                    AetherNetwork
                        .sendToServer(new PacketSendSneaking(this.mc.thePlayer.getEntityId(), this.currentSneak));
        ...
        } else if (this.mc.thePlayer.ridingEntity instanceof EntitySwet
            && !((EntitySwet) this.mc.thePlayer.ridingEntity).isFriendly()) {
                this.sneak = false;
            }
    }
