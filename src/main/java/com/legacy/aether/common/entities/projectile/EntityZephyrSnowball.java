...
public class EntityZephyrSnowball extends Entity implements IThrowableEntity
{
    ...
    public void onUpdate()
    {
        ...
        if (this.motionX * this.motionY * this.motionZ < 0.1D && this.ticksInAir > 40 && !this.world.isRemote)
        {
            this.setDead();
        }

        if (this.ticksInAir > 600 && !this.world.isRemote)
        {
            this.setDead();
        }
        ...
        RayTraceResult movingobjectposition = this.world.rayTraceBlocks(var15, vec3d1, true);
        ...
        if (movingobjectposition != null)
        {
            ...
            {
                if (movingobjectposition.entityHit instanceof EntityPlayer && ((EntityPlayer)movingobjectposition.entityHit).inventory.armorInventory.get(0).getItem() == ItemsAether.sentry_boots)
                {
                    this.setDead();
                }
